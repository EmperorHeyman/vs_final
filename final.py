...
def makeguifordecode():
    ...
    vstup = Entry(gui)
    btndec = Button(gui, fg="Blue", text="PŘEVEĎ", command=decodetorim)
    ...
    def encodefromrim():
        ...
        if(pokracuj2):
            ...
            if vypis:
                ...
                natextrome.set("Zadávej římská čísla. Jinak nebudu funkovat.")
            info.set("")
    textrim = Label(gui, text="Zadej římské číslo:")
    vstup = Entry(gui)
    b = Button(gui, fg="Blue", text="PŘEVEĎ", command=encodefromrim)
    btnmenu = Button(gui, text="Zpět na menu", command=menu)
    textrimdecode = Label(gui, textvariable=info)
    btnen = Label(gui, textvariable=natextrome)
    textrim.grid(row=0, column=0, pady=10, padx=10)
